...
struct AppDetailView: View {
    let report: NDPrivacySummary.NDApplicationSummary
    ...
    var body: some View {
                    ...
                    if report.reportPrivacyElement.count > 0 {
                        ...
                        ForEach(privacyDataAccessDateByDay, id: \.self) { dayKey in
                            ...
                            LazyVGrid(columns: [GridItem(.adaptive(minimum: 140, maximum: 200))], alignment: .leading, content: {
                                if let accessData = privacyDataSource[dayKey] {
                                    ForEach(accessData, id: \.self) { data in
                                        ZStack {
                                            HStack {
                                                imageTint(of: data)
                                                    .resizable()
        ...
        .onChange(of: report, perform: { _ in
            let token = UUID()
            buildingToken = token
